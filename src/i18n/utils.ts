...
/** Locale-aware internal link: '/' + locale prefix (Spanish only) + base. */
 ...
 * so swapping locale only ever has to add/remove the `/es` prefix.
 */
export function swapLocaleHref(targetLocale: Locale, pathname: string): string {
  ...
  const unprefixed = routePath.startsWith('/es/')
    ? routePath.slice(3)
    : routePath === '/es'
      ? '/'
      : routePath;
