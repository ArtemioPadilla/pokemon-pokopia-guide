 ...
 * (and `areas`, defined the same `${name}_${locale}` way by hand) — every real
 * `CollectionKey` ends in `_en` or `_es`, so this strips that suffix back off. A
 * typo'd name here is now a compile error instead of a runtime `getCollection`
 * failure at build time. */
type LocalizedBaseName<K> = K extends `${infer Base}_en`
  ? Base
  : K extends `${infer Base}_es`
    ? Base
    : never;
